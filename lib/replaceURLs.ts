...
export default function createAnchors(message: string): string {
  return regexReplace(message, urlRegex, function(match: string): string {
...
export function regexReplace(message: string, regex: RegExp, replace: string | ((match: string) => string)): string {
